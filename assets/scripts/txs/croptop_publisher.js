const croptopContract = async () => {
    return "0x7c129e346a52bb7d366253d983b0992119c9b39b";
}
const contractABI = [{
  "inputs": [
    {
...
const tx_collect = async (projectId, category, quantity, price, encodedIPFSUri, beneficiary, cpnBeneficiary) => {
  ...
  console.log({ projectId, posts: [post], beneficiary, cpnBeneficiary, value: bigIntPrice + (projectId == 748 ? BigInt(0) : (bigIntPrice / divisor)) });
  return await sign(await croptopContract(), contractABI, "collect", [projectId, [post], beneficiary, cpnBeneficiary, {
      value: bigIntPrice + (projectId == 748 ? BigInt(0) : (bigIntPrice / divisor)) 
  }]);
}
